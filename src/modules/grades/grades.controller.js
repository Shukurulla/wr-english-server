...
export const finalize = asyncHandler(async (req, res) => {
  const grade = await gradesService.finalizeGrade(req.params.id, req.body, req.user._id);
  return ok(res, grade);
});

export const override = asyncHandler(async (req, res) => {
  const grade = await gradesService.overrideGrade(req.params.id, req.body, req.user._id);
  return ok(res, grade);
});
